...
class static_upsample(object):
    """
    Upsample the input feature map by a scale_factor of 2

    Explanation
    -----------
    1. torch.nn.Upsample has one undesirable property, which causes problem
    in TensorRT conversion.
    2. To upsample a feature_map, nn.Upsample take the `shape/size` of a feature_map tensor
    and use them directly in upsampling with torch.nn.functional.
    3. This makes the nn.Upsample a dynamic operation so causes problem in
    TensorRT because TensorRT 7.0 doesn't support dynamic padding operations.
    """
    def __init__(self, scale_factor=2, mode='nearest'):
        """
        Initializes class with parameter values

        Parameters
        ----------
        scale_factor : int
                       scale_factor, for upscampling tensor
        mode         : str
                       upscaling method
        """
        self.scale_factor = int(scale_factor)
        self.all_modes    = ['nearest', 'linear', 'bilinear', 'bicubic', 'trilinear']
        self.mode = str(mode)

        assert self.mode in self.all_modes, "Upsample mode doesn't exist."

    def __call__(self, feature_map):
        """
        Perform upsampling function on feature map

        Parameter
        ---------
        feature_map : torch tensor
                      torch tensor, resulted from torch.conv2d operation
        ...
        """
        sh = torch.tensor(feature_map.shape)
        # typecasting int to size makes it static in onnx/tensorrt model conversion
        return F.interpolate(feature_map, size=(int(sh[2] * self.scale_factor), 
                                                int(sh[3] * self.scale_factor)), mode=self.mode)
